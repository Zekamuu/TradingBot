...
if(trainModel):
    ...
    # Get number of actions from the stock dataframe
    n_actions = 3  # Assuming actions are 'Long', 'Short', 'Do Nothing'
    
    # Get the number of state observations from the dataframe columns
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            if t + 1 < len(df):
                observation = df.iloc[t + 1].values
                reward = df.iloc[t + 1]['c'] - df.iloc[t]['c']  # Reward based on price change
                terminated = False
                truncated = False
            else:
                observation = None
